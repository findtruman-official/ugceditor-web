...
import { idlFactory as backendFactory, _SERVICE as BackendType } from '../../../declarations/backend';
import { idlFactory as findsFactory, _SERVICE as FindsType } from '../../../declarations/finds';
import { idlFactory as nftFactory, _SERVICE as NftType } from '../../../declarations/nft';
...
export class PlugWalletProvider implements WalletProvider {
  ...
  async mintStoryNft(
    ...
    if (!this.backendActor || !this.findsActor) throw new Error('Actor not available!')

    const account = this.provider.sessionManager.sessionData.principalId;

    const findsBalanceBn = new BigNumber((await this.findsActor.balanceOf(Principal.fromText(account))).toString())
    ...
    try {
      await this.findsActor.approve(Principal.fromText(this.factoryAddress), BigInt(price))
    } catch (e) {
      console.log(e)
    }
    const nftActor = await this.getNftSaleActor(id);

    const nftBalance = Number(await nftActor!.balanceOfDip721(Principal.fromText(account)))
    try {
      const res = await this.backendActor.mintNft(BigInt(id))
      while (Number(await nftActor!.balanceOfDip721(Principal.fromText(account))) === nftBalance) {
  ...
  async balanceOfStoryNft(account: string, nftName: string, storyId: string) {
    ...
    if (nftActor) {
     return Number(await nftActor.balanceOfDip721(Principal.fromText(account)))
    } else {
      return 0;
    }
  }
